...
1. **Upload PDF documents** to the persistent knowledge base
2. **Ask a question** about drilling engineering
3. **FAISS vector search** finds the most relevant passages across ALL documents
4. **LLM generates an answer** grounded in those passages (`openai/gpt-oss-120b` via the Groq API, or DeepSeek-R1:1.5b via Ollama)
5. **Sources are cited** with document name, page number, and chapter
...
├── streamlit_app.py                             # Streamlit frontend (multi-document ChatPDF)
...
│   ├── knowledge_base.py                        # Multi-document knowledge base manager
...
│   └── knowledge_base/                          # Persistent knowledge base (auto-generated)
│       ├── documents/                           # Stored PDFs (by document ID)
│       ├── indexes/
│       │   └── faiss.index                      # FAISS vector index (all documents)
│       └── metadata/
│           ├── chunks.parquet                   # Chunk metadata with document tracking
│           └── documents.csv                    # Document registry
...
The application supports a persistent multi-document knowledge base:

### Upload PDFs

- Upload PDF documents directly from the Streamlit sidebar
- Documents are automatically processed: extracted, cleaned, chunked, and embedded
- SHA256 hash prevents duplicate uploads

### Persistent Knowledge Base

- All documents, chunks, and the FAISS index are persisted locally
- Data survives app restarts — no reprocessing needed
- Stored in `implementation/knowledge_base/`

### Cross-Document Search

- Questions search across ALL indexed documents
- Sources display document name, page number, chapter, and similarity score
- Context building deduplicates across documents

### Document Management

- Sidebar shows: total documents, total pages, total chunks
- Each document shows: name, page count, chunk count, upload date
- View metadata or delete any document
- Deletion removes chunks and rebuilds the FAISS index automatically

### Chunk Metadata

Every chunk tracks:
- `document_id` -- SHA256 hash of the source PDF
...
4. **Fixed chunk size** -- 100 words / 25 overlap; not optimized per topic
5. **Groq model availability** -- Groq periodically deprecates and replaces hosted models; check [console.groq.com/docs/deprecations](https://console.groq.com/docs/deprecations) if the configured model ID stops working
